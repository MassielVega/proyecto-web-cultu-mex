...
<body>
  ...
  if (isset($_SESSION['idU'])) // el usuario esta autenticado
  {
    ...
    if ($rolUsr == "Administrador") {
      ...
      $idC = $_REQUEST['idC'];
      $c = conectarBD();
      $qry = "select * from clasificacion where idC='$idC'";
      $rs = mysqli_query($c, $qry);
      $row = $rs->fetch_assoc();
  ?>
      <center>
        <form action="proceso_modificarClasificacion.php?idC=<?php echo $row['idC']; ?> " method="POST">
          <input type="text" REQUIRED name="nombre" value="<?php echo $row['nombre']; ?>"><br>
          <input class="btn btn-dark fuenteMenu" type="submit" value="Aceptar"><br><br>
          <a class="btn btn-light fuenteMenu" href="catedralM.php">Cancelar</a>
        </form>
      </center><?php
                piedepag();
              } else {
